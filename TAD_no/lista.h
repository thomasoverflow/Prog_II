...
/**
 * @brief Operador de caminha pela lista
 * @details Recebe um ponteiro de lista e uma posição. Operador percorre a lista
 * encadeada até chegar na posição passada.
 * 
 * @param l a lista a ser percorrida, pos a posição a encontrar
 * @return Ponteiro do nó naquela posição.
 */
no_t* caminha_lista(lista_t *l,int pos);

/**
 * @brief Cria e inicializa uma estrutura de nó, retornando seu endereço
 * 
 * @details Aloca memória para uma estrutura de nó duplamente encadeado e re-
 * torna seu endereço (ponteiro). Inicialmente o nó criado é vazio, ou seja,
 * não contém nenhum dado. Caso não seja possível realizar a alocação a fun-
 * ção deve retornar NULL.
 * 
 * @return Ponteiro para o nó inicializado. Caso falhe, retorna NULL.
 */
no_t* cria_no();
